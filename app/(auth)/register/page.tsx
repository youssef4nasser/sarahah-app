...
export default function Register() {
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data: Inputs) => {
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        toast.error(
          String(
            error.response.data.errMsg || error.response.data.Error[0].message
          )
        );
      } else {
  ...
  return (
    <main className="flex-1 flex flex-col items-center justify-center w-full px-4 max-w-lg mx-auto py-8">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {errors.confirmPassword && (
              <span className="text-red-500">
                {errors.confirmPassword.message || "Password is required"}
              </span>
            )}
          </div>
          {/* gender */}
